...
def ed_rand_1_bin(np, max_gen, f, cr):
    ...
    for g in range(max_gen):
        ...
        for i in range (np):
            ...
            population = population[order]
        logging.debug("Current best internal CV score: [alpha=%s, fit_prior=%s], fitness=%s" % (population[0,0], False if population[0,1] < 0.5 else True, fitness[0]))
        # Surplus disposal
        population = population[:np]
        fitness = fitness[:np]
    return fitness[0]

def main(np, max_gen, f, cr, datfile):
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description='Hyperparameter tuning for software requirements categorization using Differential Evolution')
    ...
    ap.add_argument('--seed', dest='seed', type=int, required=True, help='Random state seed')
    ...
    main(args.np, args.max_gen, args.f, args.cr, args.datfile)